...
class ImageInferencePage(FunctionPage):
    def __init__(self, mw, model, conf_thres, device_option, parent=None):
        ...
        self.conf_thres    = conf_thres
        self.device_option = device_option
        self.folder_scan_count = 0  # GUI 启动后文件夹扫描次数计数
        self.folder_detected_total = 0  # 已完成扫描的螺栓ID累计数
    ...
    def select_image(self):
        ...
        try:
            rows, sample_orig, sample_ann, annotated_infos, _ = self.run_inference(
                [fp], scan_dir
            )
    ...
    def select_folder(self):
        ...
        ordered_names = sorted(names, key=_num_key)

        # 根据扫描次数奇偶切换正序/倒序扫描
        self.folder_scan_count += 1
        if self.folder_scan_count % 2 == 0:
            ordered_names = list(reversed(ordered_names))

        files = [os.path.join(dir_path, f) for f in ordered_names]
        if not files:
        ...
        try:
            rows, sample_orig, sample_ann, annotated_infos, unique_id_count = self.run_inference(
                files,
                scan_dir,
                progress_callback=update_progress,
                preserve_order=True,
                bolt_id_offset=self.folder_detected_total,
                reset_tracker_before=(self.folder_scan_count == 1),
                reset_tracker_after=False,
            )
            self.folder_detected_total += unique_id_count
    ...
    def run_inference(
        ...
        bolt_id_offset=0,
        reset_tracker_before=True,
        reset_tracker_after=True,
    ):
        # 根据需要重置跟踪器，避免非文件夹模式下的 ID 污染
        if reset_tracker_before:
            reset_tracker_state(self.model)
        ...
        for idx, (fp, res) in enumerate(zip(files, results_gen)):
            ...
            for det in detections:
                ...
                if isinstance(raw_id, int):
                    unique_raw_ids.add(raw_id)
                    if min_int_raw_id is None or raw_id < min_int_raw_id:
                        min_int_raw_id = raw_id
        ...
        if reset_tracker_after and hasattr(self.model, "tracker") and self.model.tracker is not None:
            try:
                self.model.tracker.tracker.reset()
            except Exception:
                pass

        effective_offset = bolt_id_offset
        if min_int_raw_id is not None and (min_int_raw_id - effective_offset) != 1:
            effective_offset = min_int_raw_id - 1

        for rec in detection_records:
            raw_id = rec["raw_id"]
            if isinstance(raw_id, int):
                stable_id = raw_id - effective_offset
            else:
                stable_id = raw_id
            ...
            detail_lines.append(
                f"{rec['image']} - ID {stable_id} - {rec['class']}({rec['confidence']:.3f})"
            )

        self.text_detail.setPlainText("\n".join(detail_lines))
        unique_id_count = len({rid for rid in unique_raw_ids if isinstance(rid, int)})
        return rows, sample_orig, sample_ann, annotated_infos, unique_id_count
